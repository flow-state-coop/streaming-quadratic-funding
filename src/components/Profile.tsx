...
function Profile() {
  ...
  const accountTokenSnapshotNative = userTokenSnapshots?.filter(
    (snapshot) => snapshot.token === ETHX_ADDRESS.toLowerCase()
  )[0];
  const accountTokenSnapshotWrapper = userTokenSnapshots?.filter(
    (snapshot) => snapshot.token === DAIX_ADDRESS.toLowerCase()
  )[0];
  const superTokenBalanceWrapper = useFlowingAmount(
    BigInt(accountTokenSnapshotWrapper?.balanceUntilUpdatedAt ?? 0),
    accountTokenSnapshotWrapper?.updatedAtTimestamp ?? 0,
    BigInt(accountTokenSnapshotWrapper?.totalNetFlowRate ?? 0)
  );
  const superTokenBalanceNative = useFlowingAmount(
    BigInt(accountTokenSnapshotNative?.balanceUntilUpdatedAt ?? 0),
    accountTokenSnapshotNative?.updatedAtTimestamp ?? 0,
    BigInt(accountTokenSnapshotNative?.totalNetFlowRate ?? 0)
  );
      ...
      >
              ...
              {formatNumberWithCommas(
                parseFloat(
                  roundWeiAmount(
                    BigInt(
                      token === Token.ETHx
                        ? accountTokenSnapshotNative?.totalOutflowRate ?? 0
                        : accountTokenSnapshotWrapper?.totalOutflowRate ?? 0
                    ) *
                      BigInt(
                        fromTimeUnitsToSeconds(
                          1,
        ...
        >
          ...
          <Card.Text className="m-0 text-gray overflow-hidden text-truncate fs-4">
            {token === Token.ETHx &&
            accountTokenSnapshotNative?.maybeCriticalAtTimestamp
              ? dayjs
                  .unix(accountTokenSnapshotNative.maybeCriticalAtTimestamp)
                  .format("MMM D, YYYY")
              : token === Token.DAIx &&
                accountTokenSnapshotWrapper?.maybeCriticalAtTimestamp
              ? dayjs
                  .unix(accountTokenSnapshotWrapper?.maybeCriticalAtTimestamp)
                  .format("MMM D, YYYY")
              : "N/A"}
          </Card.Text>
        </Stack>
